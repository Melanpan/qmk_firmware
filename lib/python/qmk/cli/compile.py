...
def compile(cli):
    ...
    # If compile keyboard are supplied, assign those
    if cli.args.keyboard:
        keyboard = cli.args.keyboard
    # config keyboard
    elif cli.config.compile.keyboard and not keyboard:
        keyboard = cli.config.compile.keyboard

    # If compile keymap are supplied, assign those
    if cli.args.keymap:
        ...
        # If user keymap are supplied, assign those
        elif cli.config.compile.keymap:
            keymap = cli.config.compile.keymap

    if cli.args.filename:
        # Parse the configurator json
        keymap = parse_configurator_json(cli.args.filename.name)

        # Generate the keymap
        keymap_path = qmk.path.keymap(keymap['keyboard'])
        cli.log.info('Creating {fg_cyan}%s{style_reset_all} keymap in {fg_cyan}%s', keymap['keymap'], keymap_path)

        # Compile the keymap
        command = compile_configurator_json(cli.args.filename.name)

        cli.log.info('Wrote keymap to {fg_cyan}%s/%s/keymap.c', keymap_path, keymap['keymap'])
